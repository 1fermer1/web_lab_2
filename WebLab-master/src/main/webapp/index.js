...
function showValueX(value) {
    var buttons = document.querySelectorAll('.x');
    buttons.forEach(function(button) {
        button.classList.remove('active');
    });

    // Установите класс "active" для нажатой кнопки
...
form.addEventListener('submit', (evt) => {
    ...
    if (isNaN(Number.parseFloat(y)) || Number.parseFloat(y) <= -5 || Number.parseFloat(y) >= 3) {
        alert('Y must be bigger than -5, but less than 3');
        return;
    }
